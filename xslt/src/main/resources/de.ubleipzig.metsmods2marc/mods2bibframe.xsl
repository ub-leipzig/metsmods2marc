...
>
    ...
    <xsl:function name="local:rdf-resource">
                ...
                <xsl:when test="$node/ancestor-or-self::mods:relatedItem">
                    <xsl:choose>
                        <xsl:when test="$node/ancestor-or-self::mods:relatedItem/mods:recordInfo/mods:recordIdentifier">
                            <xsl:value-of
                                    select="$node/ancestor-or-self::mods:relatedItem/mods:recordInfo/mods:recordIdentifier/text()"/>
                        </xsl:when>
                        <xsl:when test="$node/ancestor-or-self::mods:relatedItem/mods:identifier[not(@invalid='yes')]">
                            <xsl:value-of
                                    select="replace($node/ancestor-or-self::mods:relatedItem/mods:identifier[not(@invalid='yes')][1]/text(),' ','')"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:choose>
                                <xsl:when
                                        test="$node/ancestor-or-self::mods:mods/mods:recordInfo/mods:recordIdentifier">
                                    <xsl:value-of
                                            select="$node/ancestor-or-self::mods:mods/mods:recordInfo/mods:recordIdentifier/text()"/>
                                </xsl:when>
                                <xsl:when test="$node/ancestor-or-self::mods:mods/mods:identifier[not(@invalid='yes')]">
                                    <xsl:value-of
                                            select="replace($node/ancestor-or-self::mods:mods/mods:identifier[not(@invalid='yes')][1]/text(),' ','')"/>
                                </xsl:when>
                                <xsl:otherwise>
        ...
        <xsl:variable name="titleType">
            <xsl:choose>
                <xsl:when test="$node/@type='abbreviated'">
                    <xsl:text>abbreviatedTitle</xsl:text>
                </xsl:when>
                <xsl:when test="$node/@type='alternative' and $node/@otherType='keyTitle'">
                    <xsl:text>KeyTitle</xsl:text>
                </xsl:when>
                <xsl:when test="$node/@type='alternative'">
                    <xsl:text>titleVariation</xsl:text>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:choose>
            <xsl:when test="$node/ancestor-or-self::mods:relatedItem">
                <xsl:value-of
                        select="concat($BASEURI, '/',$id,'/',$ref-type,$ref-subType,count($node/preceding-sibling::*) +1)"/>
            </xsl:when>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="$ref-type !=''">
                        <xsl:value-of
                                select="concat($BASEURI, '/',$id,'/',$ref-type,count($node/preceding-sibling::*) +1)"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of
                                select="concat($BASEURI, '/',$id,'/',local-name($node),count($node/preceding-sibling::*) +1)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="work">
        ...
        >
            ...
            <xsl:attribute name="about" namespace="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                <xsl:choose>
                    <xsl:when test="self::mods:relatedItem">
                        <xsl:value-of select="local:rdf-resource(.,'relatedItem')"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="local:rdf-resource(.,'work')"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
        ...
        >
            <xsl:attribute name="about" namespace="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                <xsl:choose>
                    <xsl:when test="self::mods:relatedItem">
                        <xsl:value-of
                                select="concat(local:rdf-resource(.,'relatedItem'),'/','instance',count(child::*))"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="concat(local:rdf-resource(.,'work'),'/','instance',count(child::*))"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
            <xsl:apply-templates select="mods:typeOfResource" mode="uri"/>
            ...
            <xsl:apply-templates
                    select="mods:titleInfo[not(@type='alternative') and not(@type='abbreviated') and not(@type='translated')]"/>
            ...
            >
                <xsl:attribute name="about" namespace="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                    <xsl:choose>
                        <xsl:when test="self::mods:relatedItem">
                            <xsl:value-of
                                    select="concat(local:rdf-resource(.,'relatedItem'),'/','annotation',count(child::*))"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of
                                    select="concat(local:rdf-resource(.,'work'),'/','annotation',count(child::*))"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:attribute>
            ...
            >
                <xsl:attribute name="about" namespace="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                    <xsl:choose>
                        <xsl:when test="self::mods:relatedItem">
                            <xsl:value-of
                                    select="concat(local:rdf-resource(.,'relatedItem'),'heldItem',count(child::*))"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="concat(local:rdf-resource(.,'work'),'heldItem',count(child::*))"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:attribute>
    ...
    <xsl:template match="mods:titleInfo">
        ...
        <bf:title xml:lang="x-bf-sort">
            <xsl:value-of select="string-join(child::*[not(self::mods:nonSort)],' ')"/>
        </bf:title>
    </xsl:template>
    ...
    <xsl:template match="mods:title">
        ...
        <bf:titleValue>
            <xsl:if test="preceding-sibling::mods:nonSort">
                <xsl:value-of select="concat(preceding-sibling::mods:nonSort,' ')"/>
            </xsl:if>
            <xsl:value-of select="."/>
        </bf:titleValue>
    </xsl:template>
    ...
    <xsl:template match="mods:name" mode="uri">
        ...
        <xsl:variable name="relator">
            <xsl:for-each
                    select="doc('http://id.loc.gov/vocabulary/relators.madsrdf.rdf')//madsrdf:MADSScheme/madsrdf:hasTopMemberOfMADSScheme">
                <relator>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$relator/relator = mods:role/mods:roleTerm[@type='code' and @authority='marcrelator']">
                <xsl:element name="{mods:role/mods:roleTerm[@type='code' and @authority='marcrelator']}"
                             namespace="http://id.loc.gov/vocabulary/relators/" inherit-namespaces="no">
                    <xsl:attribute name="resource" namespace="http://www.w3.org/2000/01/rdf-schema#"
                                   select="local:rdf-resource(.,$type)"/>
                </xsl:element>
            </xsl:when>
            <xsl:otherwise>
                    ...
                    <xsl:when test="@eventType='distribution'">
                        <bf:distribution>
                            <bf:Provider>
                                ...
                                <xsl:apply-templates
                                        select="mods:place | mods:publisher | mods:dateIssued | mods:dateCreated | mods:dateOther | mods:copyrightDate"
                                        mode="provider"/>
                            </bf:Provider>
                        </bf:distribution>
                    </xsl:when>
                ...
                <bf:providerStatement>
                    <xsl:variable name="provider-string">
                        <xsl:if test="mods:place/mods:placeTerm[@type='text'] or mods:place/mods:placeTerm[not(@type)]">
                            <xsl:value-of
                                    select="mods:place/mods:placeTerm[@type='text'] | mods:place/mods:placeTerm[not(@type)]"/>
                        </xsl:if>
                        ...
                        <xsl:apply-templates
                                select="mods:dateIssued | mods:dateCreated | mods:dateOther | mods:copyrightDate"/>
                    </xsl:variable>
    ...
    <xsl:template match="mods:language" mode="property">
        <xsl:choose>
            <xsl:when test="@objectPart">
                <bf:language
                        rdf:resource="{concat('http://id.loc.gov/vocabulary/languages/',mods:languageTerm/text())}"/>
            </xsl:when>
            <xsl:otherwise>
                <bf:language>
                    <bf:Language>
    ...
    <xsl:template match="mods:language[@objectPart]">
        <bf:Language rdf:about="{local:rdf-resource(.,'languages')}">
            <bf:resourcePart>
                <xsl:value-of select="string(@objectPart)"/>
            </bf:resourcePart>
    ...
    <xsl:template match="mods:physicalDescription/mods:reformattingQuality" mode="property">
        <bf:note>
            <xsl:value-of select="concat('Reformatting quality: ',.)"/>
        </bf:note>
    </xsl:template>
    <xsl:template match="mods:physicalDescription/mods:internetMediaType" mode="property">
        ...
        <bf:format>
            <xsl:value-of select="concat('Internet media type: ',.)"/>
        </bf:format>
    </xsl:template>
    ...
    <xsl:template match="mods:physicalDescription/mods:digitalOrigin" mode="property">
        ...
        <bf:note>
            <xsl:value-of select="concat('Digital origin: ',.)"/>
        </bf:note>
    </xsl:template>
                        ...
                        <xsl:otherwise>
                            <bf:hasAuthority>
                                <madsrdf:Authority>
                                    ...
                                    <madsrdf:authoritativeLabel>
                                        <xsl:value-of
                                                select="string-join(mods:hierarchicalGeographic/child::text(),'--')"/>
                                    </madsrdf:authoritativeLabel>
                                    <!--
                                    ...
                                    <madsrdf:isMemberOfMADSScheme
                                            rdf:resource="http://id.loc.gov/authorities/subjects"/>
                                </madsrdf:Authority>
                            </bf:hasAuthority>
                        </xsl:otherwise>
                    </xsl:choose>
    ...
    <xsl:template match="mods:coordinates">
        <bf:cartographicCoordinates>
            <xsl:apply-templates select="text()"/>
        </bf:cartographicCoordinates>
    </xsl:template>
    <xsl:template match="mods:scale">
        <bf:cartographicScale>
            <xsl:apply-templates select="text()"/>
        </bf:cartographicScale>
    </xsl:template>
    <xsl:template match="mods:projection">
        <bf:cartographicProjection>
            <xsl:apply-templates select="text()"/>
        </bf:cartographicProjection>
    </xsl:template>
    ...
    <xsl:template name="subject-child">
        <bf:authorizedAccessPoint>
            <xsl:value-of select="normalize-space(string-join(child::*,'--'))"/>
        </bf:authorizedAccessPoint>
        <rdfs:label>
            <xsl:value-of select="normalize-space(string-join(child::*,'--'))"/>
        </rdfs:label>
        ...
        <xsl:choose>
            ...
            <xsl:when test="@authority = 'lcc'">
                <bf:classificationLcc
                        rdf:resource="{concat('http://id.loc.gov/authorities/classification/',normalize-space(.))}"/>
            </xsl:when>
            <!-- NOTE: this doesn't have a url, nor does the nlm ? -->
            <xsl:when test="@authority = 'nlm'">
                <bf:classificationNlm
                        rdf:resource="{concat('http://nlm.example.org/classification/',normalize-space(.))}"/>
            </xsl:when>
            <!-- NOTE: this doesn't have a url maybe: http://udcdata.info/ -->
            <xsl:when test="@authority = 'udc'">
                <bf:classificationUdc
                        rdf:resource="{concat('http://udc.example.org/classification/',normalize-space(.))}"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="mods:classification">
        <bf:Classification>
            ...
            <xsl:if test="@valueURI">
                <bf:classificationNumberUri rdf:resource="{@valueURI}"/>
            </xsl:if>
        </bf:Classification>
    </xsl:template>
        ...
        <bf:descriptionSource>
            <xsl:attribute name="rdf:resource">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="concat('http://id.loc.gov/vocabulary/organizations/',.)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
        </bf:descriptionSource>
    ...
    <xsl:template match="mods:recordOrigin">
        <bf:generationProcess>Converted from MODS to BIBFRAME using MODS_BIBFRAME.xsl (<xsl:value-of
        ...
        <bf:descriptionConventions>
            <xsl:attribute name="rdf:resource">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of
                                select="concat('http://id.loc.gov/vocabulary/descriptionConventions/',.)"></xsl:value-of>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
        </bf:descriptionConventions>
