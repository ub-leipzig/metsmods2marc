<xsl:stylesheet xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mods="http://www.loc.gov/mods/v3"
    ...
    <xsl:function xmlns:functx="http://www.functx.com" name="functx:substring-after-last-match"
                  as="xs:string">
...
-->
            ...
            <xsl:choose>
                ...
                <!--  UBL default  -->
                <xsl:otherwise>
                    <marc:controlfield tag="007">cr uuu---uuuuu</marc:controlfield>
                </xsl:otherwise>
            </xsl:choose>
            <marc:controlfield tag="008">
                ...
                <xsl:choose>
                    <!--  1/04 fix  -->
                    <xsl:when
                            test="mods:recordInfo/mods:recordContentSource[@authority = 'marcorg']">
                        <xsl:value-of
                ...
                <xsl:choose>
                    <xsl:when
                            test="mods:originInfo/mods:dateIssued[@point = 'start' and @encoding = 'marc']">
                        <xsl:value-of
                                select="mods:originInfo/mods:dateIssued[@point = 'start' and @encoding = 'marc']"
                        />
                    </xsl:when>
                    ...
                    <xsl:when test="mods:recordInfo/mods:recordCreationDate">
                        <xsl:value-of select="substring-before(mods:recordInfo/mods:recordCreationDate, '-')"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:text/>
                    </xsl:otherwise>
                ...
                <xsl:choose>
                    <xsl:when
                            test="mods:originInfo/mods:dateIssued[@point = 'end' and @encoding = 'marc']">
                        <xsl:value-of
                                select="mods:originInfo/mods:dateIssued[@point = 'end' and @encoding = 'marc']"
                        />
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:text>||||</xsl:text>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="mods:language/mods:languageTerm[@authority = 'iso639-2b']">
                        <xsl:value-of
                                select="mods:language/mods:languageTerm[@authority = 'iso639-2b']"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:text>|||</xsl:text>
                    </xsl:otherwise>
            ...
            <!--  default production statement -->
            <xsl:call-template name="datafield">
                <xsl:with-param name="tag">264</xsl:with-param>
                <xsl:with-param name="ind2">1</xsl:with-param>
                <xsl:with-param name="subfields">
                    <marc:subfield code="a">[Leipzig]</marc:subfield>
                    <marc:subfield code="b">[Universitätsbibliothek Leipzig]</marc:subfield>
                    <marc:subfield code="c">
                        <xsl:value-of select="substring-before(mods:recordInfo/mods:recordCreationDate, '-')"/>
                    </marc:subfield>
                </xsl:with-param>
            </xsl:call-template>
            <!--  default content statement -->
            <xsl:call-template name="datafield">
            ...
            <!--  default media statement -->
            <xsl:call-template name="datafield">
                <xsl:with-param name="tag">337</xsl:with-param>
                <xsl:with-param name="subfields">
                    <marc:subfield code="a">Computermedien</marc:subfield>
                    <marc:subfield code="b">c</marc:subfield>
                    <marc:subfield code="2">rdamedia</marc:subfield>
                </xsl:with-param>
            </xsl:call-template>
            <!--  default carrier type statement -->
            <xsl:call-template name="datafield">
    ...
    <!--  Genre elements  -->
    <xsl:template
            match="mods:genre[@authority != 'marcgt' or not(@authority)][not(parent::mods:subject)]">
    ...
    <xsl:template match="mods:originInfo">
        ...
        <!--  v3 dates 2018-01-24 omit per BM
        <xsl:if test="mods:dateModified | mods:dateCreated | mods:dateValid">
            <xsl:call-template name="datafield">
                ...
                <xsl:with-param name="subfields">
                    ...
                    <xsl:for-each
                            select="mods:dateCreated[@point = 'start'] | mods:dateCreated[not(@point)]">
                    ...
                    <xsl:for-each
                            select="mods:dateValid[@point = 'start'] | mods:dateValid[not(@point)]">
    ...
    <!--  hack for UBL language MODS impl  -->
    ...
    <xsl:template
            match="mods:form[@type = 'media'] | mods:form[@type = 'carrier'] | mods:form[@type = 'material'] | mods:form[@type = 'technique']">
        <xsl:call-template name="datafield">
            <xsl:with-param name="tag">
                <xsl:choose>
...
-->
    ...
    <xsl:template name="controlRecordInfo">
        ...
        <!-- 2018/01/19 - check if URI is in recordIdentifier field -->
        <xsl:for-each select="mods:recordInfo/mods:recordIdentifier">
            ...
            <xsl:if test="contains(.,'http')">
                ...
                <xsl:call-template name="datafield">
                    ...
                    <xsl:with-param name="subfields">
                        <marc:subfield code="3">Handschriftenkatalog</marc:subfield>
                        <marc:subfield code="u">
                            <xsl:value-of select="."/>
                        </marc:subfield>
    ...
    <xsl:template match="mods:identifier[@type = 'urn']">
        ...
        <xsl:call-template name="datafield">
            ...
            <xsl:with-param name="subfields">
                ...
                <marc:subfield code="x">Digitalisierung</marc:subfield>
                <marc:subfield code="y">Online-Zugriff</marc:subfield>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <!-- v3 location/url  -->
