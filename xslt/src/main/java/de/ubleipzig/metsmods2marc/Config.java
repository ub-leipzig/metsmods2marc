...
public class Config {
    ...
    private static void checkTrailingSlashes(final String source, final String destination) {
        if ((source.endsWith("/") && !destination.endsWith("/"))
            ...
            logger.warn(
                    "Possible mismatch between the source and destination URIs: one ends with a "
                            + "trailing "
                            +
    ...
    /**
     * returns true if the configuration is set for import.
     *
    ...
    /**
     * returns true if the configuration is set for export.
     *
    ...
    public void setXsltResource(final URI xsltresource) {
        if (xsltresource.toString().endsWith("/")) {
            this.xsltresource = URI.create(
                    xsltresource.toString().substring(0, xsltresource.toString().length() - 1));
        } else {
    ...
    public void setMap(final String[] map) {
        if (map.length == 2 && map[0] != null && map[1] != null) {
            ...
            throw new IllegalArgumentException(
                    "The map should contain the export and import baseURLs");
        }
    }
